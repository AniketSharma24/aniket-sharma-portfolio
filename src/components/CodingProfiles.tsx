...
import { 
  Github, ExternalLink, Code, AlertCircle, Terminal, MessageSquare
} from "lucide-react";
    ...
    >
      <div className="container mx-auto max-w-4xl">
        <div className="text-center mb-16">
          <h2 className="text-sm font-medium text-primary mb-2 animate-on-scroll opacity-0">MY PRESENCE</h2>
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto text-balance animate-on-scroll opacity-0" style={{ animationDelay: "400ms" }}>
            Connect with me on various coding platforms and explore my solutions to algorithmic challenges.
          </p>
        </div>
          ...
          {profiles.map((profile, index) => (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-background flex items-center justify-center mb-4 border border-border/70 shadow-sm">
                {platformIcons[profile.platform] || <ExternalLink className="w-5 h-5" />}
              </div>
              <h4 className="font-serif font-semibold mb-1">{profile.platform}</h4>
              {profile.username && (
